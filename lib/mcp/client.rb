...
module MCP
  ...
  class Client
    ...
    def initialize(mcp_server_configs: [])
      @servers = mcp_server_configs.map { |config| MCP::ServerFactory.create(config) }
      @tool_cache = {}
    end
    ...
    def call_tool(tool_name, parameters)
      ...
      raise MCP::Errors::ToolNotFound, "Tool '#{tool_name}' not found" unless tool

      # Find the server that owns this tool
      server = find_server_for_tool(tool)
      raise MCP::Errors::ServerNotFound, "No server found for tool '#{tool_name}'" unless server
