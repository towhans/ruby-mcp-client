...
module MCP
  ...
  class ServerSSE < ServerBase
    ...
    # @raise [MCP::Errors::ServerError] if server returns an error
    # @raise [MCP::Errors::TransportError] if response isn't valid JSON
    # @raise [MCP::Errors::ToolCallError] for other errors during tool listing
    def list_tools
      ...
      begin
        ...
        unless response.is_a?(Net::HTTPSuccess)
          raise MCP::Errors::ServerError, "Server returned error: #{response.code} #{response.message}"
        end

        data = JSON.parse(response.body)
        @tools = data['tools'].map do |tool_data|
          MCP::Tool.from_json(tool_data)
        end
      rescue JSON::ParserError => e
        raise MCP::Errors::TransportError, "Invalid JSON response from server: #{e.message}"
    ...
    # @raise [MCP::Errors::ServerError] if server returns an error
    # @raise [MCP::Errors::TransportError] if response isn't valid JSON
    # @raise [MCP::Errors::ToolCallError] for other errors during tool execution
    ...
    # @raise [MCP::Errors::ConnectionError] if connection fails
    def connect
      ...
      raise MCP::Errors::ConnectionError, "Failed to connect to MCP server at #{@base_url}: #{e.message}"
    end
